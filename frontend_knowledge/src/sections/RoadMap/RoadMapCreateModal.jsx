...
function RoadMapCreateModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (event) => {
        ...
        try {
            const response = await API.post(
                "http://127.0.0.1:8000/roadmap_create/",
                {                 
                    title,                    
                    description,
                }
  ...
  return (
    <div className="modal-overlay">
      ...
      <div className="modal-content">        
        <h3>Создание дорожной карты</h3>
        
        {/* начало формы */}
        <form onSubmit={handleSubmit} style={{ marginBottom: '1rem' }}>
                

                <label htmlFor="id_title">Заголовок дорожной карты: </label>
                <input 
